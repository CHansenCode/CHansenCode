...
export const New = ({ ...props }) => {
  ...
  async function handleSubmit(e, title, formData) {
    e.preventDefault();

    try {
      dispatch(api.createSlide(formData, title));
      clearFormAndClose(e);
    } catch (error) {
      console.log(error);
    }
  ...
  return (
    <li className={css.item_new}>
      {open ? (
        <Form
          padding="0.75rem"
          onSubmit={e => handleSubmit(e, formData, props.formData)}
        >
          <TypeInput
            label="Name of new Slide"
            value={formData}
          ...
          <Grid gap="1rem">
            <Button
              ...
              padding="0.15rem"
              disabled={!valid}
              onClick={e => handleSubmit(e, formData, props.formData)}
            />
            <Button
              text="CLOSE"
              padding="0.15rem"
